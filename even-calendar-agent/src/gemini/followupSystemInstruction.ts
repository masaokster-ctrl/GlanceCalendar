...
const MISSING_FIELD_DESCRIPTIONS: Record<MissingField, string> = {
  ...
  date: '日付が不明です',
  start_time: '開始時刻が不明です(終日の予定であれば「終日」のように回答してもよい)',
...
export function buildFollowupSystemInstruction(params: BuildFollowupSystemInstructionParams): string {
  ...
  return [
    ...
    "  - allDaySignal: 今回の回答が「終日」「一日中」「有給」「休暇」「休み」等、時刻を示さず1日(または",
    "    複数日)全体を指す語であれば'all_day'。実際の時刻(例:「15時から」)で回答すれば'timed'とし、",
    ...
    '- 「やめる」「キャンセル」「やっぱりいい」など、予定登録自体を取りやめる意図が明確な発話はresultType="cancelled"とすること。',
    ...
    '- 複数の異なる予定が含まれる場合、または複数日にわたる期間と「毎日」等の繰り返しを示す語が同時に',
