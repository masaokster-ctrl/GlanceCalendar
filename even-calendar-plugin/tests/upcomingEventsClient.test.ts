...
import { fetchUpcomingEvents } from '../src/upcomingEventsClient'
...
function validResultBody(): unknown {
  return { schemaVersion: '1', mode: 'upcoming', timeZone: 'Asia/Tokyo', events: [], truncated: false }
}
...
describe('fetchUpcomingEvents', () => {
  ...
  it('returns success with the parsed result for a 200 response', async () => {
    ...
    const outcome = await fetchUpcomingEvents({ ...BASE_PARAMS, signal: new AbortController().signal })
    expect(outcome.kind).toBe('success')
    if (outcome.kind === 'success') {
      expect(outcome.result.mode).toBe('upcoming')
    }
  })

  it('sends a GET request with the required headers and limit query param, no body', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(200, validResultBody()))
    vi.stubGlobal('fetch', fetchMock)
    await fetchUpcomingEvents({ ...BASE_PARAMS, limit: 5, signal: new AbortController().signal })
  ...
  it('maps 403 to forbidden', async () => {
    ...
    expect((await fetchUpcomingEvents({ ...BASE_PARAMS, signal: new AbortController().signal })).kind).toBe('forbidden')
  })

  it('maps 429 to rate_limited', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(null, { status: 429 })))
    expect((await fetchUpcomingEvents({ ...BASE_PARAMS, signal: new AbortController().signal })).kind).toBe('rate_limited')
  })

  it('maps 504 to timeout', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(null, { status: 504 })))
    expect((await fetchUpcomingEvents({ ...BASE_PARAMS, signal: new AbortController().signal })).kind).toBe('timeout')
  })

  it('maps other non-ok statuses to failed', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(null, { status: 502 })))
    expect((await fetchUpcomingEvents({ ...BASE_PARAMS, signal: new AbortController().signal })).kind).toBe('failed')
  })
  ...
  it('returns aborted immediately if the signal is already aborted', async () => {
    ...
    controller.abort()
    const outcome = await fetchUpcomingEvents({ ...BASE_PARAMS, signal: controller.signal })
  ...
  it('returns aborted when the caller cancels mid-flight', async () => {
    ...
    const controller = new AbortController()
    const promise = fetchUpcomingEvents({ ...BASE_PARAMS, signal: controller.signal })
    controller.abort()
  ...
  it('returns timeout when the internal timer fires (not caused by caller cancellation)', async () => {
    ...
    const outcome = await fetchUpcomingEvents({ ...BASE_PARAMS, signal: new AbortController().signal, timeoutMs: 20 })
